...
func main() {
	...
	instruction := widget.NewLabel("Нажмите на пункт меню, когда появится задание")
	content := container.NewVBox(instruction, output)
...
func createMenuItem(label string, output *widget.Entry, instruction *widget.Label, parent ...string) *fyne.MenuItem {
	...
	item := fyne.NewMenuItem(label, func() {
		depth := len(strings.Split(fullPath, "→"))
		fmt.Printf("Нажат пункт: %s | Глубина: %d\n", fullPath, depth)
...
func showChart(results []TrialResult) {
	...
	for _, r := range results {
		...
		bars = append(bars, container.NewVBox(
			lbl,
			container.NewHBox(barActual, canvas.NewText(fmt.Sprintf("Фактическое нажатие: %d мс", r.ActualTimeMs), theme.ForegroundColor())),
			container.NewHBox(barHick, canvas.NewText(fmt.Sprintf("по Хика: %.0f мс", r.HickTimeMs), theme.ForegroundColor())),
			widget.NewSeparator(),
		))
	}
